...
export default function ConfirmButton ({mensagem , onClick , disabled}){
    ...
    return (
        <section className="w-full">
            <button onClick={onClick} disabled={disabled} className={` w-[20rem] h-[4rem] py-5 rounded-3xl text-xl/tight font-semibold ${tema === 'dark' ? 'bg-sky-400 hover:bg-sky-600' : 'bg-red-500 hover:bg-red-700 '}  text-white  cursor-pointer disabled:${disabledClasses} disabled:cursor-not-allowed`}>{mensagem}</button>
        </section>
    )
